...
import pickle
print(pickle.HIGHEST_PROTOCOL)#.highest_protocol)
with open ('summary_vector', 'rb') as fp:
    #pickle.dump('summary_vector',fp )#, protocol=2)
    vec_summaries = pickle.load(fp)

with open ('text_vector', 'rb') as fp:
    #pickle.dump('text_vector',fp )
    vec_texts = pickle.load(fp)
with open ('vocabstore', 'rb') as fp:
    vocab_limit = pickle.load(fp)

with open ('embdeding_store', 'rb') as fp:
...
vec_texts_reduced = []
vec_summaries_reduced=vec_summaries
vec_texts_reduced=vec_texts
...
with tf.Session() as sess: # Start Tensorflow Session
    ...
    best_val_acc=0
    display_step = 1
    save_path = saver.save(sess, "/chkp/mode_check.ckpt")
